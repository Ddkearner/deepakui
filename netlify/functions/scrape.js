...
export const handler = async (event, context) => {
    ...
    // Define the scraping logic as a promise
    ...
    // Safety timeout: Return potentially incomplete HTML after 9 seconds rather than 502
    const timeoutPromise = new Promise(resolve => {
        setTimeout(() => {
            resolve({
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    html: `<html><body><div style="font-family:sans-serif;padding:2rem;text-align:center;">
                        <h2>Partially Loaded</h2>
                        <p>The website took too long to analyze. Please try again or try a simpler page.</p>
                        <p>Original URL: <a href="${url}">${url}</a></p>
                    </div></body></html>`
                })
            });
        }, 9000); // 9 seconds hard limit (Netlify allows 10s)
    });
